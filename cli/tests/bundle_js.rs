...
fn bundle_hello_contains_runtime_and_entry() {
    ...
    assert!(
        ...
        "println runtime definition missing"
    );
    assert!(
        js.contains("println(\"Hello, Artcode!\")"),
        "entry code missing"
    );
}
...
fn bundle_inlines_imported_module() {
    ...
    cmd.args([
        "build",
        work.path().join("main.art").to_str().unwrap(),
        "--target",
        "js",
        "--bundle",
        "--out",
    ...
    assert!(
        ...
        "inlined module function missing"
    );
    assert!(
        !js.contains("import *"),
        "import statement should be suppressed in bundle"
    );
}
...
fn bundle_no_duplicate_modules() {
    ...
    std::fs::write(
        work.path().join("lib.art"),
        "import util;\nfunc wrap(x) { return id(x); }",
    )
    .expect("write lib");
    std::fs::write(
        work.path().join("main.art"),
    ...
    assert_eq!(
        count, 1,
        "module util was inlined {count} times, expected 1"
    );
}
