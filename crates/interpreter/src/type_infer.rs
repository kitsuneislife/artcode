use core::{Program, Expr, Stmt, ArtValue, Type};
...
impl TypeEnv {
    pub fn new() -> Self { Self { types: HashMap::new(), vars: HashMap::new() } }
    fn id(expr: &Expr) -> usize { expr as *const _ as usize }
    fn set(&mut self, expr: &Expr, t: Type) { self.types.insert(Self::id(expr), t); }
    pub fn set_var(&mut self, name: &str, t: Type) { self.vars.insert(name.to_string(), t); }
    pub fn get_var(&self, name: &str) -> Option<&Type> { self.vars.get(name) }
    pub fn get(&self, expr: &Expr) -> Option<&Type> { self.types.get(&Self::id(expr)) }
}
...
impl<'a> TypeInfer<'a> {
    ...
    fn visit_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::Let { name, initializer, .. } => { let t = self.infer_expr(initializer); self.tenv.set_var(&name.lexeme, t); },
            Stmt::Block { statements } => { for s in statements { self.visit_stmt(s); } },
            Stmt::If { condition, then_branch, else_branch } => {
                self.infer_expr(condition);
                self.visit_stmt(then_branch);
                if let Some(e) = else_branch { self.visit_stmt(e); }
            },
            Stmt::EnumDecl { name, variants } => {
                let mut map = HashMap::new();
                for (v, params) in variants { map.insert(v.lexeme.clone(), params.as_ref().map(|p| p.len())); }
    ...
    fn infer_expr(&mut self, expr: &Expr) -> Type {
        ...
        let t = match expr {
            ...
            Logical { left, right, .. } => { self.infer_expr(left); self.infer_expr(right); Type::Bool },
            Variable { name } => self.tenv.get_var(&name.lexeme).cloned().unwrap_or(Type::Unknown),
            Call { callee, arguments } => { self.infer_expr(callee); for a in arguments { self.infer_expr(a); } Type::Unknown },
            StructInit { .. } => Type::Unknown,
            EnumInit { name, variant, values } => {
                let mut val_types = Vec::new();
                for v in values { val_types.push(self.infer_expr(v)); }
                if let Some(n) = name {
                    if let Some(vmap) = self.enums.get(&n.lexeme) {
                        if let Some(expected) = vmap.get(&variant.lexeme) {
                            ...
                            self.diags.push(Diagnostic::new(
                                DiagnosticKind::Type,
                                format!("Unknown enum variant '{}' for enum '{}'.", variant.lexeme, n.lexeme),
                                Span::new(variant.start, variant.end, variant.line, variant.col)));
                            Type::Enum(n.lexeme.clone())
                        }
                    } else { Type::Enum(n.lexeme.clone()) }
                } else {
...
fn value_type(v: &ArtValue) -> Type {
    ...
    ArtValue::String(_) => Type::String,
        ...
        ArtValue::Array(vals) => {
            if let Some(first) = vals.first() { Type::Array(Box::new(value_type(first))) } else { Type::Array(Box::new(Type::Unknown)) }
        },
