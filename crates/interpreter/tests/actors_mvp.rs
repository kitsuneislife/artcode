...
fn actor_mailbox_fifo_and_backpressure_and_scheduler() {
    ...
    let call1 = Stmt::Expression(Expr::Call {
        ...
        arguments: vec![
            Expr::Literal(core::ast::ArtValue::Int(actor_id as i64)),
            Expr::Literal(core::ast::ArtValue::Int(1)),
        ],
    });
    ...
    let call2 = Stmt::Expression(Expr::Call {
        ...
        arguments: vec![
            Expr::Literal(core::ast::ArtValue::Int(actor_id as i64)),
            Expr::Literal(core::ast::ArtValue::Int(2)),
        ],
    });
    ...
    let mailbox_vals: Vec<i64> = interp
        ...
        .iter()
        .filter_map(|env| {
            if let core::ast::ArtValue::Int(n) = &env.payload {
                Some(*n)
            } else {
                None
            }
    ...
    interp
        ...
        .unwrap();
    let aid_sender = match interp.last_value.clone().unwrap() {
        core::ast::ArtValue::Actor(id) => id,
        core::ast::ArtValue::Int(n) => n as u32,
        _ => panic!(),
    };
    interp
        .interpret(vec![Stmt::SpawnActor { body: vec![] }])
        .unwrap();
    let aid_target = match interp.last_value.clone().unwrap() {
        core::ast::ArtValue::Actor(id) => id,
        core::ast::ArtValue::Int(n) => n as u32,
        _ => panic!(),
    };
    ...
    interp
        .interpret(vec![Stmt::Expression(Expr::Call {
            ...
            arguments: vec![
                Expr::Literal(core::ast::ArtValue::Int(aid_target as i64)),
                Expr::Literal(core::ast::ArtValue::Int(99)),
            ],
        })])
        .unwrap();
    ...
    interp
        .interpret(vec![Stmt::SpawnActor { body: vec![] }])
        .unwrap();
    let aid_pri = match interp.last_value.clone().unwrap() {
        core::ast::ArtValue::Actor(id) => id,
        core::ast::ArtValue::Int(n) => n as u32,
        _ => panic!(),
    };
    ...
    interp
        .interpret(vec![Stmt::Expression(Expr::Call {
            ...
            arguments: vec![
                Expr::Literal(core::ast::ArtValue::Int(aid_pri as i64)),
                Expr::Literal(core::ast::ArtValue::Int(1)),
                Expr::Literal(core::ast::ArtValue::Int(0)),
            ],
        })])
        .unwrap();
    interp
        .interpret(vec![Stmt::Expression(Expr::Call {
            ...
            arguments: vec![
                Expr::Literal(core::ast::ArtValue::Int(aid_pri as i64)),
                Expr::Literal(core::ast::ArtValue::Int(2)),
                Expr::Literal(core::ast::ArtValue::Int(10)),
            ],
        })])
        .unwrap();
    let pri_vals: Vec<i64> = interp
        .actors
        .get(&aid_pri)
        .unwrap()
        .mailbox
        ...
        .iter()
        .map(|e| {
            if let core::ast::ArtValue::Int(n) = &e.payload {
                *n
            } else {
                -1
            }
    ...
    interp2
        .interpret(vec![Stmt::Expression(Expr::Call {
            ...
            arguments: vec![
                Expr::Literal(core::ast::ArtValue::Int(aid2 as i64)),
                Expr::Literal(core::ast::ArtValue::Int(10)),
            ],
        })])
        .unwrap();
    ...
    assert!(matches!(
        res1,
        core::ast::ArtValue::Bool(true) | core::ast::ArtValue::Optional(_)
    ));
    interp2
        .interpret(vec![Stmt::Expression(Expr::Call {
            type_args: None,
            ...
            arguments: vec![
                Expr::Literal(core::ast::ArtValue::Int(aid2 as i64)),
                Expr::Literal(core::ast::ArtValue::Int(11)),
            ],
        })])
        .unwrap();
    ...
    let body = vec![
        ...
        Stmt::Expression(Expr::Call {
            ...
            arguments: vec![Expr::Literal(core::ast::ArtValue::Int(2))],
        }),
    ];
    interp3.interpret(vec![Stmt::SpawnActor { body }]).unwrap();
    let aid3 = match interp3.last_value.clone().unwrap() {
        core::ast::ArtValue::Actor(id) => id,
        core::ast::ArtValue::Int(n) => n as u32,
        _ => panic!(),
    };
    ...
    interp4
        .interpret(vec![Stmt::SpawnActor { body: vec![] }])
        .unwrap();
    let aid4 = match interp4.last_value.clone().unwrap() {
        core::ast::ArtValue::Actor(id) => id,
        core::ast::ArtValue::Int(n) => n as u32,
        _ => panic!(),
    };
    ...
    interp4
        .interpret(vec![Stmt::Expression(Expr::Call {
            ...
            arguments: vec![
                Expr::Literal(core::ast::ArtValue::Int(aid4 as i64)),
                Expr::Literal(core::ast::ArtValue::Int(123)),
            ],
        })])
        .unwrap();
    // prepare actor body that calls actor_receive_envelope and stores to global for inspection
    let body = vec![Stmt::Let {
        pattern: core::ast::MatchPattern::Variable(core::Token::dummy("m")),
        ty: None,
    ...
    interp4
        .interpret(vec![Stmt::SpawnActor { body: body.clone() }])
        .unwrap();
    let receiver = match interp4.last_value.clone().unwrap() {
        core::ast::ArtValue::Actor(id) => id,
        core::ast::ArtValue::Int(n) => n as u32,
        _ => panic!(),
    };
