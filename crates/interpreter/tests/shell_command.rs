...
fn runs_shell_command_statement() {
    ...
    match interp
        .last_value
        .clone()
        .expect("shell should publish result")
    {
        core::ast::ArtValue::EnumInstance {
            ...
            match &values[0] {
                core::ast::ArtValue::String(s) => {
                    assert!(
                        s.contains("shell_ok"),
                        "stdout payload should include shell output"
                    )
                }
...
fn shell_command_is_blocked_in_pure_mode() {
    ...
    assert!(
        interp.interpret(program).is_ok(),
        "interpreter should not hard-fail"
    );
    let diags = interp.take_diagnostics();
    assert!(
        diags.iter().any(|d| d
            .message
            .contains("Operation 'shell' is not allowed in --pure mode")),
        ...
        .expect("shell should publish result")
    {
        core::ast::ArtValue::EnumInstance {
            ...
            match &values[0] {
                core::ast::ArtValue::String(s) => {
                    assert!(
                        s.contains("SHELL_PIPE_OK"),
                        "pipeline stdout should be captured"
                    )
                }
                other => panic!("expected string payload, got {:?}", other),
            }
