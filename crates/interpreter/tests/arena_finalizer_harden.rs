...
fn finalizer_promotions_counted_per_arena() {
    ...
    assert!(
        ...
        "interpret program in arena_finalizer_harden.rs failed"
    );
    ...
    assert!(
        per > 0
            ...
            || interp.debug_get_global("promoted2").is_some(),
        "expected promotions attributed to arena"
    );
    assert!(
        interp.debug_check_invariants(),
        "invariants failed after promotions attribution"
    );
}
...
fn finalize_arena_idempotent_and_safe() {
    ...
    let attach = vec![Stmt::Expression(Expr::Call {
        ...
        arguments: vec![
            Expr::Literal(ArtValue::HeapComposite(core::ast::ObjHandle(oid))),
            Expr::Variable {
                name: core::Token::dummy("alloc_once"),
            },
        ],
    ...
    assert!(
        interp.interpret(attach).is_ok(),
        "interpret attach in arena_finalizer_harden.rs failed"
    );
    // finalize twice
    interp.debug_finalize_arena(aid);
    assert!(
        interp.debug_check_invariants(),
        "invariants failed after first finalize"
    );
    interp.debug_finalize_arena(aid);
    assert!(
        interp.debug_check_invariants(),
        "invariants failed after second finalize"
    );
}
