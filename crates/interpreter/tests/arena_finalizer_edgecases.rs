...
fn finalizer_promotes_handles_from_arena() {
    ...
    let program = vec![
        Stmt::Function { name: core::Token::dummy("fp"), params: vec![], return_type: None,
    ...
    assert!(interp.debug_check_invariants(), "invariants failed after finalizer promotions");
}
...
fn finalizer_allocation_inside_arena_does_not_break_finalize() {
    ...
    let program = vec![
        Stmt::Function { name: core::Token::dummy("allocf"), params: vec![], return_type: None,
...
fn weak_and_unowned_inside_arena_invalidated_on_finalize() {
    ...
    let w = interp.debug_heap_register_in_arena(ArtValue::HeapComposite(core::ast::ObjHandle(id)), aid);
    ...
    assert!(interp.debug_heap_upgrade_weak(id).is_none(), "weak upgrade unexpectedly succeeded after finalize");
}
