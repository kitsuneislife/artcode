...
fn finalizer_promote_other_survives() {
    ...
    let program = vec![
        ...
        Stmt::Function { name: core::Token::dummy("promote_b"), params: vec![], return_type: None,
            body: std::rc::Rc::new(Stmt::Block { statements: vec![Stmt::Let { name: core::Token::dummy("survivor"), ty: None, initializer: Expr::Literal(ArtValue::HeapComposite(core::ast::ObjHandle(b))) }] }), method_owner: None },
    ...
    assert!(interp.debug_check_invariants(), "invariants broken after finalize ordering test");
}
...
fn mutual_finalizers_promote_each_other_safe() {
    ...
    let program = vec![
        Stmt::Function { name: core::Token::dummy("prom_a"), params: vec![], return_type: None,
            body: std::rc::Rc::new(Stmt::Block { statements: vec![Stmt::Let { name: core::Token::dummy("x"), ty: None, initializer: Expr::Literal(ArtValue::HeapComposite(core::ast::ObjHandle(a))) }] }), method_owner: None },
        Stmt::Function { name: core::Token::dummy("prom_b"), params: vec![], return_type: None,
            body: std::rc::Rc::new(Stmt::Block { statements: vec![Stmt::Let { name: core::Token::dummy("y"), ty: None, initializer: Expr::Literal(ArtValue::HeapComposite(core::ast::ObjHandle(b))) }] }), method_owner: None },
    ...
    let promoted_any = interp.get_finalizer_promotions() > 0 || interp.finalizer_promotions_per_arena.get(&aid).cloned().unwrap_or(0)>0;
    assert!(ga.is_some() || gb.is_some() || promoted_any, "mutual finalizers failed to promote any handle");
    assert!(interp.debug_check_invariants(), "invariants failed after mutual finalizers test");
}
