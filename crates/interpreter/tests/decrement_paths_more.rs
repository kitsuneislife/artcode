...
fn finalizer_promotes_multiple_temporaries_to_root() {
    ...
    assert!(
        interp.debug_get_global("kept1").is_some(),
        "kept1 não promovido pelo finalizer"
    );
    assert!(
        interp.debug_get_global("kept2").is_some(),
        "kept2 não promovido pelo finalizer"
    );
    // Métrica: verificar que pelo menos uma promoção de handles foi contabilizada
    assert!(
        interp.get_finalizer_promotions() > 0,
        "expected finalizer_promotions > 0 after finalizer run"
    );
}
...
fn nested_performant_return_emits_diagnostic() {
    ...
    assert!(
        ...
        "esperado diagnóstico ao retornar objeto de arena de bloco performant"
    );
}
...
fn field_assign_prev_arena_triggers_finalizer() {
    ...
    let aid = interp.debug_create_arena();
    let child_arena =
        interp.debug_heap_register_in_arena(ArtValue::Array(vec![ArtValue::Int(7)]), aid);
    interp.debug_define_global(
        "child_arena",
        ArtValue::HeapComposite(core::ast::ObjHandle(child_arena)),
    );
    ...
    interp.debug_define_global(
        "child_plain",
        ArtValue::HeapComposite(core::ast::ObjHandle(child_plain)),
    );
    ...
    let s_id = interp.debug_heap_register(ArtValue::StructInstance {
        struct_name: "S2".to_string(),
        fields,
    });
    ...
    let program = vec![
        ...
        Stmt::Expression(Expr::Call {
            ...
            arguments: vec![
                Expr::Variable {
                    name: core::Token::dummy("child_arena"),
                },
                Expr::Variable {
                    name: core::Token::dummy("fin_prev"),
                },
    ...
    let new_s = interp.debug_heap_register(ArtValue::StructInstance {
        struct_name: "S2".to_string(),
        fields: new_fields,
    });
    ...
    assert!(
        interp.debug_get_global("prev_flag").is_some(),
        "finalizer do campo anterior não executou"
    );
}
