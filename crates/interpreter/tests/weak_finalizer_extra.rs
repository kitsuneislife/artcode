...
fn finalizer_creates_multiple_handles() {
    ...
    interp.debug_define_global("owner1", ArtValue::HeapComposite(core::ast::ObjHandle(owner1)));
    let owner2 = interp.debug_heap_register(ArtValue::Array(vec![ArtValue::Int(2)]));
    interp.debug_define_global("owner2", ArtValue::HeapComposite(core::ast::ObjHandle(owner2)));
            ...
            body: std::rc::Rc::new(Stmt::Block {
                statements: vec![
                    Stmt::Let {
                        ...
                        ty: None,
                        initializer: Expr::Variable { name: core::Token::dummy("owner1") },
                    },
                    Stmt::Let {
                        name: core::Token::dummy("saved2"),
                        ty: None,
                        initializer: Expr::Variable { name: core::Token::dummy("owner2") },
                    },
                ],
            }),
            method_owner: None,
    ...
    assert!(interp.debug_get_global("saved1").is_some(), "saved1 não criado");
    assert!(interp.debug_get_global("saved2").is_some(), "saved2 não criado");
    assert!(interp.debug_heap_contains(owner1), "owner1 deveria ser preservado");
    assert!(interp.debug_heap_contains(owner2), "owner2 deveria ser preservado");
}
...
fn finalizer_promotes_multiple_to_root_from_arena() {
    ...
    // arena obj removido
    assert!(!interp.debug_heap_contains(arena_obj), "objeto de arena ainda presente");
    ...
    assert!(interp.debug_heap_contains(outside), "objeto externo deveria ser preservado");
}
