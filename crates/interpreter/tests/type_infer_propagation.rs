...
fn shadowing_restores_outer_type() {
    ...
    assert!(
        type_diags.is_empty(),
        "expected none after shadow restore, found: {:?}",
        type_diags
    );
}
...
fn function_param_propagation() {
    ...
    assert!(
        type_diags.is_empty(),
        "expected no type diags for function param propagation, found: {:?}",
        type_diags
    );
}
...
fn unknown_var_still_warns() {
    ...
    assert!(
        !type_diags.is_empty(),
        "expected diagnostic for unknown variable payload"
    );
}
