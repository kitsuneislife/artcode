...
fn return_arena_escape_diagnostic() {
    ...
    let found = diags.iter().any(|d| d.message.contains("Attempt to bind arena object"));
    assert!(found, "esperado diagnóstico de escape ao bindar objeto de arena para escopo externo");
}
...
fn finalizer_allocs_temporary_promoted() {
    ...
    let program = vec![
        Stmt::Function {
            ...
            body: std::rc::Rc::new(Stmt::Block {
                statements: vec![Stmt::Let {
                    ...
                    ty: None,
                    initializer: Expr::Variable { name: core::Token::dummy("temp") },
                }],
            }),
            method_owner: None,
        },
        Stmt::Expression(Expr::Call {
            ...
            arguments: vec![Expr::Variable { name: core::Token::dummy("target") }, Expr::Variable { name: core::Token::dummy("fin_alloc") }],
        }),
    ];
    ...
    assert!(interp.debug_get_global("kept").is_some(), "finalizer não criou 'kept'");
