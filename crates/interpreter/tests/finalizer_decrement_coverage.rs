...
fn rebind_triggers_finalizer_and_clears_handles() {
    ...
    let program = vec![
        ...
        Stmt::Expression(Expr::Call {
            ...
            arguments: vec![Expr::Variable { name: core::Token::dummy("a") }, Expr::Variable { name: core::Token::dummy("fin") }],
        }),
        // rebind a
...
fn return_of_arena_object_is_reported() {
    ...
    let program = vec![
        Stmt::Function {
            ...
            params: vec![],
            return_type: None,
            body: std::rc::Rc::new(Stmt::Block { statements: vec![Stmt::Return { value: Some(Expr::Literal(ArtValue::HeapComposite(core::ast::ObjHandle(id)))) }] }),
            method_owner: None,
        },
...
fn performant_arena_finalization_promotes_and_cleans() {
    ...
    assert!(interp.debug_check_invariants(), "invariants failed after finalize_arena");
}
