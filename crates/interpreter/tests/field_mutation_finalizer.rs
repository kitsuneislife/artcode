...
fn field_mutation_runs_finalizer_and_decrements() {
    ...
    interp.register_struct_for_test(
        "Pair",
        vec![
            (core::Token::dummy("left"), "Any".to_string()),
            (core::Token::dummy("right"), "Any".to_string()),
        ],
    );
    ...
    let pair = core::ast::ArtValue::StructInstance {
        ...
        fields: {
            ...
            m.insert(
                "left".to_string(),
                ArtValue::HeapComposite(core::ast::ObjHandle(old_obj)),
            );
        ...
        Stmt::Function {
            ...
            body: std::rc::Rc::new(Stmt::Block {
                statements: vec![Stmt::Let {
                    pattern: core::ast::MatchPattern::Variable(core::Token::dummy("old_gone")),
                    ty: None,
    ...
    let new_pair = core::ast::ArtValue::StructInstance {
        ...
        fields: {
            ...
            m.insert(
                "left".to_string(),
                ArtValue::HeapComposite(core::ast::ObjHandle(new_obj)),
            );
            m.insert("right".to_string(), ArtValue::Int(0));
            m
        },
    ...
    assert!(
        flag.is_some(),
        "finalizer não executou após mutação de campo"
    );
    // Forçar sweep caso não tenha ocorrido
    interp.debug_sweep_dead();
    assert!(
        !interp.debug_heap_contains(old_obj),
        "old_obj deveria ter sido removido"
    );
}
