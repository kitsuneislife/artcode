...
impl Diagnostic {
    pub fn new(kind: DiagnosticKind, message: impl Into<String>, span: Span) -> Self {
        Self { kind, message: message.into(), span, notes: vec![] }
    }
...
pub fn format_diagnostic(source: &str, d: &Diagnostic) -> String {
    ...
    use std::fmt::Write;
    let kind_str = match d.kind { DiagnosticKind::Lex => "lex", DiagnosticKind::Parse => "parse", DiagnosticKind::Type => "type", DiagnosticKind::Runtime => "runtime", DiagnosticKind::Internal => "internal" };
    let _ = writeln!(out, "{} error ({}:{}): {}", kind_str, d.span.line, d.span.col, d.message);
    ...
    let caret_line: String = (1..col).map(|_| ' ').collect::<String>() + "^" + &"~".repeat(underline_len.saturating_sub(1));
        let _ = writeln!(out, "{}", caret_line);
    }
    for note in &d.notes { let _ = writeln!(out, "note: {}", note); }
    out
}
