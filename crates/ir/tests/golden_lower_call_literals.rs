...
fn golden_lower_call_literals() {
    ...
    let callee_params = vec![FunctionParam {
        name: param_name.clone(),
        ty: None,
    }];
    let callee_body = Stmt::Return {
        value: Some(Expr::Variable {
            name: param_name.clone(),
        }),
    };
    let _callee = Stmt::Function {
        name: callee_name.clone(),
        params: callee_params,
        return_type: Some("i64".to_string()),
        body: std::rc::Rc::new(Stmt::Block {
            statements: vec![callee_body],
        }),
        method_owner: None,
    };
    ...
    let call_expr = Expr::Call {
        ...
        arguments: vec![Expr::Literal(core::ast::ArtValue::Int(42))],
    };
    let caller_body = Stmt::Return {
        value: Some(call_expr),
    };
    let caller = Stmt::Function {
        name: caller_name,
        params: caller_params,
        return_type: Some("i64".to_string()),
        body: std::rc::Rc::new(Stmt::Block {
            statements: vec![caller_body],
        }),
        method_owner: None,
    };
    ...
    assert!(
        text.contains("call") || text.contains("ConstI64"),
        "expected call or ConstI64 in caller lowering, got: {}",
        text
    );
}
