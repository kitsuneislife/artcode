use ir::{Sig, call_jit_fn};

// helper: get function pointer as usize for a known function type

extern "C" fn zero(out: *mut i64) -> i64 {
    unsafe {
        *out = 42;
    }
    0
}
extern "C" fn one(out: *mut i64, x: i64) -> i64 {
    unsafe {
        *out = x + 1;
    }
    0
}
extern "C" fn two(out: *mut i64, a: i64, b: i64) -> i64 {
    unsafe {
        *out = a + b;
    }
    0
}

#[test]
fn call_zero() {
    let p = zero as extern "C" fn(*mut i64) -> i64 as usize;
    let r = call_jit_fn(p, Sig::I64_0, &[]).expect("call");
    assert_eq!(r, 42);
}

#[test]
fn call_one() {
    let p = one as extern "C" fn(*mut i64, i64) -> i64 as usize;
    let r = call_jit_fn(p, Sig::I64_1, &[5]).expect("call");
    assert_eq!(r, 6);
}

#[test]
fn call_two() {
    let p = two as extern "C" fn(*mut i64, i64, i64) -> i64 as usize;
    let r = call_jit_fn(p, Sig::I64_2, &[2, 3]).expect("call");
    assert_eq!(r, 5);
}

#[test]
fn wrong_arity_errors() {
    let p = zero as extern "C" fn(*mut i64) -> i64 as usize;
    assert!(call_jit_fn(p, Sig::I64_1, &[]).is_err());
}
