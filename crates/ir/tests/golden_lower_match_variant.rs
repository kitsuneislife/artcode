...
fn golden_lower_match_variant() {
    ...
    let arm0_pat = MatchPattern::EnumVariant {
        enum_name: Some(Token::dummy("Color")),
        variant: variant_token.clone(),
        params: Some(vec![
            MatchPattern::Binding(Token::dummy("r")),
            MatchPattern::Binding(Token::dummy("g")),
            MatchPattern::Binding(Token::dummy("b")),
        ]),
    };
    let arm0_guard = None;
    let arm0_body = Stmt::Return {
    ...
    let arm1_guard = None;
    let arm1_body = Stmt::Return {
        value: Some(Expr::Literal(ArtValue::Int(0))),
    };

    let cases = vec![
        (arm0_pat, arm0_guard, arm0_body),
        (arm1_pat, arm1_guard, arm1_body),
    ];
