...
fn golden_lower_nested_if() {
    ...
    let params = vec![];
    let inner_then = Stmt::Return {
        value: Some(Expr::Literal(core::ast::ArtValue::Int(1))),
    };
    let inner_else = Stmt::Return {
        value: Some(Expr::Literal(core::ast::ArtValue::Int(2))),
    };
    let inner_if = Stmt::If {
        condition: Expr::Literal(core::ast::ArtValue::Bool(false)),
        then_branch: Box::new(inner_then),
        else_branch: Some(Box::new(inner_else)),
    };

    let outer_else = Stmt::Return {
        value: Some(Expr::Literal(core::ast::ArtValue::Int(3))),
    };
    let outer_if = Stmt::If {
        condition: Expr::Literal(core::ast::ArtValue::Bool(true)),
    ...
    assert_eq!(
        text, expected,
        "nested-if lowering differs from expected golden output\n\nexpected:\n{}\n\nactual:\n{}",
        expected, text
    );
}
