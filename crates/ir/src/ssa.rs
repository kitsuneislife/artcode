...
pub fn rename_temps(func: &mut Function) {
    ...
    let is_candidate =
        |s: &str| -> bool { s.starts_with('%') && s.starts_with(&local_prefix) && !is_renamed(s) };
    ...
    for i in 0..blocks.len() {
        ...
        for instr in new_block.iter_mut() {
            match instr {
                ...
                | Instr::Mul(_d, a, b)
                | Instr::Div(_d, a, b) => {
                    if is_candidate(a) && incoming.iter().any(|(v, _)| v == a) {
                        *a = phi_dest.clone();
                    }
                    if is_candidate(b) && incoming.iter().any(|(v, _)| v == b) {
                        *b = phi_dest.clone();
                    }
                }
