...
fn set_state_uses_schedule() {
    ...
    assert!(js.contains("__schedule("), "expected __schedule call, got:\n{js}");
    assert!(js.contains("function set_count("), "expected set_count function, got:\n{js}");
}
...
fn memo_recomputed_inside_schedule() {
    ...
    assert!(js.contains("doubled ="), "expected memo recomputation inside schedule, got:\n{js}");
}
...
fn on_mount_called_via_tick() {
    ...
    assert!(js.contains("tick("), "expected tick() for on_mount, got:\n{js}");
    assert!(js.contains("__run_mount("), "expected __run_mount call, got:\n{js}");
}
